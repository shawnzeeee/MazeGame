...
public class TileManager {
    ...
    public void getTileImage() {
        ...
        try {
            // road
            tile[0] = new Tile();
            String roadURL = "/tiles/road00.png";
            InputStream roadIS = getClass().getResourceAsStream(roadURL);
            tile[0].image = ImageIO.read(roadIS);

            // wall
            tile[1] = new Tile();
            String wallURL = "/tiles/wall.png";
            InputStream wallIS = getClass().getResourceAsStream(wallURL);
            tile[1].image = ImageIO.read(wallIS);
            tile[1].collision = true;
            // grass
            tile[2] = new Tile();
            String grassURL = "/tiles/grass01.png";
            InputStream grassIS = getClass().getResourceAsStream(grassURL);
            tile[2].image = ImageIO.read(grassIS);

            //end
            tile[9] = new Tile();
            String endURL = "/tiles/end.png";
            InputStream endIS = getClass().getResourceAsStream(endURL);
            tile[9].image = ImageIO.read(endIS);
