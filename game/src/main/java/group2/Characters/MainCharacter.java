...
public class MainCharacter extends Character {
    ...
    public MainCharacter(int x, int y, int width, int height, KeyHandler keyHandler, GamePanel gp) {
        ...
        // arrays!\n"); 
        walkingUpFrames = loadFrames("sprites/MainCharacter/mcUp", 6);
        walkingDownFrames = loadFrames("sprites/MainCharacter/mcDown", 6);
        walkingLeftFrames = loadFrames("sprites/MainCharacter/mcLeft", 6);
        walkingRightFrames = loadFrames("sprites/MainCharacter/mcRight", 6);
    ...
    private BufferedImage[] loadFrames(String path, int frameCount) {
        ...
        try {
            for (int i = 0; i < frameCount; i++) {
                ...
                // ".png\n");
                //URL framePath = getClass().getResource(path + (i + 1) + ".png");
                String framePath = path + (i + 1) + ".png";
                InputStream is = this.getClass().getResourceAsStream(framePath);
                frames[i] = ImageIO.read(is);
                //frames[i] = new ImageIcon(imageUrl);
                // testing
